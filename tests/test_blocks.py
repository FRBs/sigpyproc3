...
class TestFilterbankBlock:
    def test_block_noheader(self) -> None:
        rng = np.random.default_rng()
        data = rng.normal(size=(128, 1024))
        with pytest.raises(TypeError):
            FilterbankBlock(data)  # type: ignore[call-arg]
    ...
    def test_downsample(self, filfile_8bit_1: str) -> None:
        ...
        np.testing.assert_equal(
            new_data.shape,
            (data.shape[0] // ffactor, data.shape[1] // tfactor),
        )
        np.testing.assert_equal(new_data.header.nchans, data.header.nchans // ffactor)
        np.testing.assert_equal(
            new_data.header.nsamples,
            data.header.nsamples // tfactor,
        )
    ...
    def test_dedisperse_valid_samples(self, filfile_8bit_1: str) -> None:
        ...
        np.testing.assert_equal(
            dm_data.shape[1],
            data.shape[1] - data.header.get_dmdelays(dm).max(),
        )
