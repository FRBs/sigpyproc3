...
class TestFilterbank(object):
    def test_compute_stats(self, filfile_4bit):
        fil = FilReader(filfile_4bit)
        fil.compute_stats()
        assert isinstance(fil.chan_stats, stats.ChannelStats)

    def test_compute_stats_basic(self, filfile_4bit):
        fil = FilReader(filfile_4bit)
    ...
    def test_read_chan(self, filfile_4bit):
        fil = FilReader(filfile_4bit)
        tim = fil.read_chan(5)
        np.testing.assert_equal(tim.dtype, np.float32)
        np.testing.assert_equal(tim.size, fil.header.nsamples)
    ...
    def test_apply_channel_mask(self, filfile_4bit, tmpfile):
        fil = FilReader(filfile_4bit)
        chanmask = np.random.randint(2, size=fil.header.nchans)
    ...
    def test_extract_samps(self, filfile_4bit, tmpfile):
        nsamps = 1024
        fil = FilReader(filfile_4bit)
    ...
    def test_extract_chans(self, filfile_4bit):
        fil = FilReader(filfile_4bit)
