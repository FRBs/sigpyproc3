...
void foldTim(float* buffer, double* result, int* counts, double tsamp,
...
void rfft(float* buffer, float* result, int size) {
    fftwf_plan plan;
    plan = fftwf_plan_dft_r2c_1d(size, buffer, (fftwf_complex*)result, 
                                 FFTW_ESTIMATE);
    fftwf_execute(plan);
    fftwf_destroy_plan(plan);
}

void resample(float* input, float* output, int nsamps, float accel,
              float tsamp) {
