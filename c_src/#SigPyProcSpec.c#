	      ...
	      float endfreq)
{
  ...
  while(rindex/2<nsamps){
    ...
    for (ii = 0; ii < numread_old; ii++) {
      ...
      outbuffer[ii*2+windex]=oldinbuf[ii*2]/sqrt(mean_old+slope*((numread_old+numread_new) / 2.0 - ii));
      outbuffer[ii*2+1+windex]=oldinbuf[ii*2+1]/sqrt(mean_old+slope*((numread_old+numread_new) / 2.0 - ii));
    }
